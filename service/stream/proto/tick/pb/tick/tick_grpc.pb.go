...
const (
	StreamingService_StreamTick_FullMethodName = "/tick.StreamingService/StreamTick"
)
...
type StreamingServiceClient interface {
	StreamTick(ctx context.Context, in *StreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Tick], error)
}
...
func (c *streamingServiceClient) StreamTick(ctx context.Context, in *StreamRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[Tick], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &StreamingService_ServiceDesc.Streams[0], StreamingService_StreamTick_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type StreamingService_StreamTickClient = grpc.ServerStreamingClient[Tick]
...
type StreamingServiceServer interface {
	StreamTick(*StreamRequest, grpc.ServerStreamingServer[Tick]) error
...
func (UnimplementedStreamingServiceServer) StreamTick(*StreamRequest, grpc.ServerStreamingServer[Tick]) error {
	return status.Errorf(codes.Unimplemented, "method StreamTick not implemented")
}
...
func _StreamingService_StreamTick_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StreamRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(StreamingServiceServer).StreamTick(m, &grpc.GenericServerStream[StreamRequest, Tick]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type StreamingService_StreamTickServer = grpc.ServerStreamingServer[Tick]
...
var StreamingService_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "tick.StreamingService",
	HandlerType: (*StreamingServiceServer)(nil),
	Methods:     []grpc.MethodDesc{},
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "StreamTick",
			Handler:       _StreamingService_StreamTick_Handler,
			ServerStreams: true,
		},
	},
	Metadata: "tick/tick.proto",
}
